...
현재 프로젝트에서 페이지네이션은 첫 조회에 특정 수의 유저를 조회하고 이후 어느정도 스크롤하면 그 다음 유저 조회가 필요했다.
...
각 방식의 본질과 trade-off를 이해한 뒤, UX 목표에 맞춰 선택하여 프로젝트에 적용하기 위해 offset기반 페이지네이션과 cursor기반 페이지네이션을 비교했다.

### offset 기반 페이지네이션

`offset` 기반 페이지네이션은 `limit`을 정해놓은 후 `offset`이 n일때 데이터베이스가 내부적으로 n건을 스킵한 후 `limit`만큼 읽는다.
...
커서를 통한 필터링을 id값으로 하게 되었을때 정렬 기준이 닉네임이면, 이전 페이지에서 정렬되었던 레코드가 누락되거나, 이전에 이미 표시된 레코드가 중복되어 표시될 수 있다.
...
결과적으로 `alex` (id=30)는 첫번째, 두번째 페이지에 중복되어 표시된다.
...
이를 해결하기 위해, 기존에 id로 사용하던 커서를 닉네임으로 변경하여 정렬 기준과 동일하게 변경한 후, 페이징 결과 인터페이스를 추상화 했다.
...
숫자까지 완벽하게 정렬하기 위한 방법으로 여러가지가 있지만, 많은 방법들 중 가상 컬럼을 생성하지 않고 패딩을 쓰지 않는 방법 중에 **정규식으로 숫자 부분만 뽑아서 CAST**하는 방식을 생각했다.
...
아래의 결과를 보면 포스트맨으로 요청만 보내봐도 응답 지연 차이가 눈에 띄게 차이난다.
...
사실 닉네임의 문자가 같은데 숫자가 다른경우가 빈번한 경우가 아니기 때문에 숫자는 크게 영향이 없다.
그렇기 때문에, 다른 방법을 사용하더라도 추후 Artillery를 통해 응답 지연시간을 체크하여 부하가 테스트 후 절충안을 도입해야한다.

---

## Artillery 개요

offset기반 페이지네이션을 cursor기반 페이지네이션과 비교 했을 때 발생하는 응답 지연시간 차이를 측정하기 위해 Artillery를 사용했다.
...
- **phases**
  ...
  - **rampTo**: duration동안 arrivalRate를 rampTo에 정의된 수 까지 늘린다.
...
현재 테스트는 사용자 검색 시 페이징 처리에 대한 테스트로, 한 사용자가 반복해서 무한 스크롤하여 검색 요청을 보내야하기 때문에 루프를 도는 시나리오를 작성했다.

위의 artillery 개요와 같이 yaml파일로 테스트 스크립트를 작성하고 전 후처리를 위해 사용자 정의 함수를 타입스크립트로 구현했다.

사용자 한명 당 1~2초 간격으로 50번의 무한 스크롤하는 시나리오로 테스트를 진행했다. 1회 조회 시 20개의 레코드를 가져오는 테스트로 50번 진행할 시 마지막 조회 시 1000개의 레코드를 스킵해야한다.

아래의 테스트 스크립트를 베이스로 phases의 vuser수는 테스트마다 유동적으로 변경했다.
...
500명의 사용자가 50번의 무한스크롤을 할 경우, 평균적적으로 7ms로 차이가 없었다.
...
유의미한 차이를 확인하기에는 사용자나 스크롤수가 적다고 판단하여, vuser와 count를 늘려 테스트를 진행했다.
...
미미한 차이지만 비율로 따지면 28% 차이로 cursor기반 페이징이 응답 지연시간이 빨랐다.

최대 지연시간은 offset 기반 684ms, cursor 기반 815ms로 cursor 기반이 더 느리게 나왔다.
그러나 최대 지연시간을 제외한 백분위수를 보면 전체적으로 cursor기반 페이징이 지연시간이 빠른걸 볼 수 있다.
...
다른 백분위수도 비슷한 양상의 차이를 보였고, 평균 초당 요청 처리수도 cursor 기반이 **약 40%** 가량 높았다.
...
- 총 vuser 수 500
  - **단계 1**: 20s 동안 vuser 100 생성
  - **단계 2**: 15s 동안 vuser 400 생성
- vuser 1 당 2초 간격으로 200번의 무한 스크롤

테스트 1에서 사용자를 늘린 것과 비교 했을때 테스트 4가 평균 지연시간에서 가장 큰 차이를 보였다
...
offset 수가 늘어났을 때 평균 지연시간에서 **약 3배 가량**으로 가장 큰 차이를 보였다.
...
2. 평균 지연시간 기준, 50회 스크롤할 시 사용자 500명일 때는 아예 차이가 없었으나, 1500명부터 28%의 성능 차이를 보였고 3000명일 때는 241%의 차이를 보였다.
3. 사용자가 **6배 늘어났을때**는 241%의 차이를 보였는데, offset이 **4배 늘어났을때** 287%로 비교적 큰 폭으로 지연시간이 늘어났다
